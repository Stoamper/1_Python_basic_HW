...
class Storage:
    ...
    @staticmethod
    def equipment_quantity(eq_model, quantity):
        try:
            if type(quantity) is not int:
                raise TypeError
        except TypeError:
            print('Количество должно быть целым числом! Измените данные. Указанная вами информация не введена')
            exit()
        equipment_quantity[eq_model] = quantity
        return f'Информация по количеству техники на складе: {equipment_quantity}'
...
print(sc)
pr_q = st.equipment_quantity('HP PCL400', 1)
# pr_q = st.equipment_quantity('Konica SC1000', 'four')     # пример для проверки работы исключения
print(pr_q)